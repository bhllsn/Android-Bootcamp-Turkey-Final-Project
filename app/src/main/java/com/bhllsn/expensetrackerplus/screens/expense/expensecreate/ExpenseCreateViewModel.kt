    ...
    fun createExpense(type: Int, description: String, currencyType: Int, costText: String)
    {
        try
        {
            if(type != -1 && description != "" && costText != "")
            {
                ...
                if(cost >= 0)
                {
                    val currencyTRY: Float = sharedPreferences.getFloat(PREFERENCE_TRY, 10.02F)
                    val currencyUSD = sharedPreferences.getFloat(PREFERENCE_USD, 1.22F)
                    val currencyGBP = sharedPreferences.getFloat(PREFERENCE_GBP, 0.87F)
